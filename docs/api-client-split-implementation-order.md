...
- [x] Add public client shell routes for `client2/client3`: `/`, `/blog`, `/blog/[slug]`.
- [x] Generate per-client `robots.txt`, `sitemap.xml`, `rss.xml`, `llms.txt`, `llms-full.txt`, `site.webmanifest` from BFF + central API.
- [x] Export `packages/site-config` via package entrypoint and expose `llmContext` in public site config API.
...
Current execution priority after the bridge slices:

1. Reddit admin data plane
   - [x] Add `GET /v1/admin/reddit/targets`.
   - [x] Add `POST /v1/admin/reddit/targets`.
   - [x] Add `PUT /v1/admin/reddit/targets/:id`.
   - [x] Add `DELETE /v1/admin/reddit/targets/:id`.
   - [x] Add `GET /v1/admin/reddit/runs`.
   - [x] Add `GET /v1/admin/reddit/runs/:id`.
   - [x] Add `GET /v1/admin/reddit/gaps`.
   - [x] Add `POST /v1/admin/reddit/gaps/:id/approve`.
   - [x] Add `POST /v1/admin/reddit/gaps/:id/reject`.
   - [x] Add `POST /v1/admin/reddit/gaps/auto-filter`.
   - [x] Add BFF aliases for `/api/reddit/*` that target the new API routes.
2. YouTube admin data plane
   - [x] Add `GET /v1/admin/youtube/overview`.
   - [x] Add `GET /v1/admin/youtube/targets`.
   - [x] Add `POST /v1/admin/youtube/targets`.
   - [x] Add `PUT /v1/admin/youtube/targets/:id`.
   - [x] Add `DELETE /v1/admin/youtube/targets/:id`.
   - [x] Add `GET /v1/admin/youtube/runs`.
   - [x] Add `GET /v1/admin/youtube/runs/:id`.
   - [x] Add `GET /v1/admin/youtube/gaps`.
   - [x] Add `POST /v1/admin/youtube/gaps/:id/approve`.
   - [x] Add `POST /v1/admin/youtube/gaps/:id/reject`.
   - [x] Add `POST /v1/admin/youtube/gaps/auto-filter`.
   - [x] Add BFF aliases for `/api/youtube/*` that target the new API routes.
3. Brand Clarity management slice before jobs
   - [x] Add `site_id` strategy for all `bc_*` tables.
   - [x] Add `GET/PUT /v1/admin/bc/settings`.
   - [x] Add `GET/POST /v1/admin/bc/projects`.
   - [x] Add `GET/PUT/DELETE /v1/admin/bc/projects/:id`.
   - [x] Add `PUT /v1/admin/bc/projects/:id/documentation`.
   - [x] Add `GET/POST /v1/admin/bc/projects/:id/channels`.
   - [x] Add `PUT/DELETE /v1/admin/bc/projects/:id/channels/:channelId`.
   - [x] Add `POST /v1/admin/bc/projects/:id/channels/confirm-all`.
   - [x] Add `GET /v1/admin/bc/projects/:id/videos`.
   - [x] Add `PUT /v1/admin/bc/projects/:id/videos/:videoId`.
   - [x] Add `POST /v1/admin/bc/projects/:id/videos/add-manual`.
4. Distributed job migration after read/write parity
   - [x] Move Reddit `start/status/stream` off `globalThis` into `app_jobs`.
   - [x] Move YouTube `start/status/stream` off `globalThis` into `app_jobs`.
   - [~] Move Brand Clarity parse/scrape/select/generate into `app_jobs`.
   - [x] Add first central Brand Clarity worker topic: `bc-scrape`.
   - [ ] Replace EventEmitter-only streaming with DB/job-events-backed SSE or polling.

Additional tenantization progress:

- [x] Add `site_id` strategy for Reddit tables (`reddit_targets`, `reddit_scrape_runs`, `reddit_posts`, `reddit_extracted_gaps`).
- [x] Add `site_id` strategy for YouTube tables (`yt_targets`, `yt_scrape_runs`, `yt_comments`, `yt_extracted_gaps`).
- [x] Add `site_id` strategy for Brand Clarity tables (`bc_projects`, `bc_target_channels`, `bc_target_videos`, `bc_comments`, `bc_extracted_pain_points`, `bc_settings`, `bc_iterations`, `bc_iteration_selections`, `bc_landing_page_variants`, `bc_pain_clusters`).
- [x] Scope central Reddit/YouTube API queries and inserts by `siteSlug`.
- [x] Scope central Brand Clarity management API queries and inserts by `siteSlug`.
- [x] Pass `siteId` through Reddit/YouTube worker payloads into scraper scripts.
- [x] Pass `siteId` through Brand Clarity scripts for new child rows created during pipeline execution.

Current operational reality:

- `client1` remains untouched as legacy baseline.
- `client2` and `client3` run as separate BFF runtimes with central API plus legacy fallback.
- Central API covers core content, Reddit data plane, YouTube data plane, Brand Clarity management slice, and first execution bridges.
- Worker runner currently supports: `geo`, `draft`, `reddit`, `youtube`, `bc-scrape`.
- Largest unfinished blocks are now: remaining Brand Clarity worker topics, Social Hub execution migration, and SSR admin DB decoupling.

Known remaining admin SSR DB pages:

- About `23` admin pages still read DB directly instead of going through API/BFF.
- Highest-value next decoupling candidates: `src/pages/admin/reddit/*`, `src/pages/admin/youtube/*`, then Brand Clarity management pages.
