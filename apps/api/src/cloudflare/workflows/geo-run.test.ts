...
import { executeGeoRunWorkflow } from './geo-run.ts';
...
test('executeGeoRunWorkflow reserves, executes, and finalizes the job', async () => {
  ...
  const result = await executeGeoRunWorkflow(
    {
      jobId: '41',
      payload: {},
      siteId: 7,
    ...
    },
    {
      ...
      runGeoJob: async () => ({
        ...
        gapsFound: 3,
        queriesProcessed: 9,
        runAt: new Date('2026-03-27T10:05:00.000Z'),
      }),
      step,
    },
  );
  ...
  assert.equal(result.result.queriesProcessed, 9);
