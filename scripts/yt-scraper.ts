...
async function run() {
  ...
  // Load existing commentIds to dedup
  ...
  for (const target of targets) {
    ...
    try {
      ...
      const rawItems = items as unknown as ApifyComment[];
      log(`[YT] Fetched ${rawItems.length} comments from Apify`);

      // Filter: new cids only, top-level comments, meaningful length
      ...
      // Track inserted cids to prevent future dupes within same run
      ...
      // LLM analysis in chunks
      const videoTitle = newItems[0]?.title ?? target.label;
        ...
        if (gaps.length > 0) {
          await db.insert(ytExtractedGaps).values(
            gaps.map(gap => ({
              ...
              sourceVideoId: target.videoId ?? (newItems[0]?.videoId ? String(newItems[0].videoId).substring(0, 20) : null),
