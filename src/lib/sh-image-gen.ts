...
function buildRetroBCard(opts: SocialImageOptions, w: number, h: number): string {
  ...
  return `<div style="width:${w}px;height:${h}px;background:${c.bg};display:flex;flex-direction:column;justify-content:center;align-items:center;padding:80px;font-family:sans-serif;">
    <div style="border:3px solid ${c.gold};padding:60px;display:flex;flex-direction:column;gap:40px;width:100%;box-sizing:border-box;">
      ...
      <div style="font-size:30px;color:#e2e8f0;line-height:1.6;">${opts.bodyText.slice(0, 220)}</div>
...
function buildPainPointStory(opts: SocialImageOptions, w: number, h: number): string {
  ...
  return `<div style="width:${w}px;height:${h}px;background:linear-gradient(180deg,${c.bg} 0%,#0f172a 100%);display:flex;flex-direction:column;justify-content:space-between;padding:100px 80px;font-family:sans-serif;">
    <div style="color:${c.teal};font-size:26px;font-weight:600;letter-spacing:4px;">PAIN POINT</div>
    <div style="display:flex;flex-direction:column;gap:48px;">
      <div style="font-size:68px;font-weight:900;color:white;line-height:1.1;">${opts.hookLine}</div>
      <div style="font-size:34px;color:#94a3b8;line-height:1.5;">${opts.bodyText.slice(0, 160)}</div>
    </div>
...
function buildTipCard(opts: SocialImageOptions, w: number, h: number): string {
  ...
  return `<div style="width:${w}px;height:${h}px;background:${c.bg};display:flex;flex-direction:column;padding:80px;font-family:sans-serif;">
    <div style="background:${c.teal};color:white;font-size:22px;font-weight:700;padding:12px 24px;border-radius:6px;align-self:flex-start;margin-bottom:48px;">PRO TIP</div>
    <div style="font-size:54px;font-weight:800;color:white;line-height:1.2;flex:1;">${opts.hookLine}</div>
...
function buildIgMinimal(opts: SocialImageOptions, w: number, h: number): string {
  const c = { ...DEFAULT_COLORS, ...opts.brandColors };
  return `<div style="width:${w}px;height:${h}px;background:#0d1117;display:flex;flex-direction:column;justify-content:space-between;padding:96px 80px;font-family:sans-serif;">
    <div style="display:flex;align-items:center;gap:16px;">
      <div style="width:48px;height:4px;background:${c.teal};border-radius:2px;"></div>
      <div style="font-size:20px;color:${c.teal};font-weight:600;letter-spacing:3px;">FRINTER</div>
    </div>
    <div style="display:flex;flex-direction:column;gap:40px;">
      <div style="font-size:62px;font-weight:900;color:#f8fafc;line-height:1.15;">${opts.hookLine.slice(0, 80)}</div>
...
function buildIgQuoteGradient(opts: SocialImageOptions, w: number, h: number): string {
  const c = { ...DEFAULT_COLORS, ...opts.brandColors };
  return `<div style="width:${w}px;height:${h}px;background:linear-gradient(135deg,${c.bg} 0%,#1a0b2e 50%,#0f2027 100%);display:flex;flex-direction:column;justify-content:center;align-items:center;padding:80px;font-family:sans-serif;text-align:center;">
    <div style="font-size:140px;color:${c.violet};opacity:0.2;line-height:0.7;margin-bottom:32px;">"</div>
    <div style="font-size:58px;font-weight:900;color:white;line-height:1.2;margin-bottom:48px;">${opts.hookLine.slice(0, 100)}</div>
...
function buildIgTipList(opts: SocialImageOptions, w: number, h: number): string {
  ...
  const tipRows = sentences.length > 0
    ? sentences.map((s, i) =>
        `<div style="display:flex;align-items:flex-start;gap:24px;">
          <div style="background:${c.teal};color:white;font-size:22px;font-weight:700;min-width:48px;height:48px;border-radius:50%;display:flex;align-items:center;justify-content:center;">${i + 1}</div>
          <div style="font-size:28px;color:#e2e8f0;line-height:1.5;">${s}</div>
        </div>`
      ).join('')
  ...
  return `<div style="width:${w}px;height:${h}px;background:${c.bg};display:flex;flex-direction:column;padding:80px;font-family:sans-serif;">
    <div style="margin-bottom:56px;">
      <div style="font-size:20px;font-weight:600;color:${c.gold};letter-spacing:4px;margin-bottom:20px;">QUICK WINS</div>
      <div style="font-size:54px;font-weight:900;color:white;line-height:1.15;">${opts.hookLine.slice(0, 60)}</div>
    </div>
